...
def show_yolo_pose(kps, img, cls=None):
    colors = [
        (255, 0, 0),  # 头到肩部
        (0, 255, 0),  # 左手
        (0, 0, 255),  # 右手
    ...
    # openpose
    # connections = [[0, 15], [0, 16], [0, 1],
    #             [5, 6], [6, 7],
    #             [2, 3], [3, 4],
    #             [2, 1], [1, 5], [1, 8], [8, 9], [8, 12],
    #             [12, 13], [13, 14],
    #             [9, 10], [10, 11]]
    # LR = [1, 1, 1, 2, 2, 3, 3, 4, 4, 4, 4, 4, 5, 5, 6, 6]
    ...
    for joint in angle_joints:
        ...
        # )
    # 显示当前的类比
    # cv2.putText(img, f"{cls}", (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
    return img


def show_openpose_pose(kp_list, img_list, video_writer, cls=None, ball_position=None):
    colors = [
        (255, 0, 0),  # 头到肩部
        (0, 255, 0),  # 左手
        (0, 0, 255),  # 右手
    ...
    angle_joints = [
    (5, 6, 7, "L Elbow", (114, 255, 250)),  # 左肘
    (2, 3, 4, "R Elbow", (231, 237, 62)),  # 右肘
    (12, 13, 14, "L Knee", (0, 117, 255)),  # 左膝
    (9, 10, 11, "R Knee", (193, 182, 255))  # 右膝
]
    suggest_angle_range = [
    (90, 170),  # 左肘
    (90, 170),  # 右肘
    ...
    user_picture = cv2.imread("user/tai.jpg")
...
def get_openpose_keypoint(video_path:Path, ball_data:pd.DataFrame, model_list):
    ...
    if len(keypoint) > 1:
        ...
        ball_x = ball_data.loc[ball_data['Frame'] == strike_position, 'X'].values[0]
        ball_y = ball_data.loc[ball_data['Frame'] == strike_position, 'Y'].values[0]
        k_mean = np.mean(keypoint[:, [1, 8],:2], axis=1)
        distances = np.linalg.norm(k_mean - np.array([ball_x, ball_y]), axis=1)
        player_index = np.argmin(distances)
        keypoint = keypoint[player_index]
        keypoint_list.append(keypoint)
        frame_list.append(frame)
        ball_position.append([ball_x, ball_y])
    else:
        keypoint_list.append(keypoint[0])
    ...
    for i in range(strike_position_in_video - 1, -1, -1):
        ...
        if len(keypoint) > 1:
            # 取离上一帧球员最近的人作为球员
            # distances = np.linalg.norm(keypoint[:,:2] - player_positon_in_strike[:2], axis=1)
            k_mean_new = np.mean(keypoint[:, [1, 8],:2], axis=1)
            k_mean_old = np.mean(keypoint_list[0][[1, 8],:2], axis=0)
            # print(k_mean_new.shape, k_mean_old.shape)
            distances = np.linalg.norm(k_mean_new - k_mean_old, axis=1)
            player_index = np.argmin(distances)
            keypoint_list.insert(0, keypoint[player_index])
        else:
            keypoint_list.insert(0, keypoint[0])
    ...
    for i in range(strike_position_in_video + 1, int(cap.get(cv2.CAP_PROP_FRAME_COUNT))):
        ...
        if len(keypoint) > 1:
            ...
            # distances = np.linalg.norm(keypoint[:,:2] - player_positon_in_strike[:2], axis=1)
            k_mean_new = np.mean(keypoint[:, [1, 8],:2], axis=1)
            k_mean_old = np.mean(keypoint_list[-1][[1, 8],:2], axis=0)
            distances = np.linalg.norm(k_mean_new - k_mean_old, axis=1)
            player_index = np.argmin(distances)
            keypoint_list.append(keypoint[player_index])
        else:
            keypoint_list.append(keypoint[0])
        frame_list.append(frame)
...
def get_yolo11_keypoint(video_path:Path, ball_data:pd.DataFrame, yolo_model):
    # 先提取击球帧的人，若检测到多于一个人则取里球最近的人作为球员
    _, start_frame, end_frame, strike_position = video_path.stem.split('_')
    start_frame, end_frame , strike_position = int(start_frame), int(end_frame), int(strike_position)
    strike_position_in_video = strike_position - start_frame
    ...
    result = yolo_model(source=frame, verbose=False)[0]  # predict on an image
    keypoint_list = []
    frame_list = []
    ...
    if len(result.keypoints.xy) == 0:
        print(f"No keypoint detected in frame {strike_position_in_video} of video {video_path.name}, skipping...")
        cv2.imwrite(video_path.parent / Path(video_path.stem + '_no_keypoint.png'), frame)
        return None
    if len(result.keypoints.xy) > 1:
        # 取离球最近的人作为球员
        ball_x = ball_data.loc[ball_data['Frame'] == strike_position, 'X'].values[0]
        ball_y = ball_data.loc[ball_data['Frame'] == strike_position, 'Y'].values[0]
        k_mean = np.mean(result.keypoints.xy.cpu().numpy(), axis=1)
        distances = np.linalg.norm(k_mean - np.array([ball_x, ball_y]), axis=1)
        player_index = np.argmin(distances)
        keypoint_list.append(result.keypoints.xy[player_index].cpu().numpy())
        frame_list.append(frame)
    else:
        keypoint_list.append(result.keypoints.xy[0].cpu().numpy())
    ...
    for i in range(strike_position_in_video - 1, -1, -1):
        ...
        if len(result.keypoints.xy) > 1:
            # 取离上一帧球员最近的人作为球员
            k_mean_new = np.mean(result.keypoints.xy.cpu().numpy(), axis=1)
            k_mean_old = np.mean(keypoint_list[0], axis=0)
            distances = np.linalg.norm(k_mean_new - k_mean_old, axis=1)
            player_index = np.argmin(distances)
            keypoint_list.insert(0, result.keypoints.xy[player_index].cpu().numpy())
        else:
            keypoint_list.insert(0, result.keypoints.xy[0].cpu().numpy())
        frame_list.insert(0, frame)
    # 从击球帧开始逐帧向后读取，获取球员的关节点
    for i in range(strike_position_in_video + 1, int(cap.get(cv2.CAP_PROP_FRAME_COUNT))):
        cap.set(cv2.CAP_PROP_POS_FRAMES, i)
        ...
        if len(result.keypoints.xy) > 1:
            # 取离上一帧球员最近的人作为球员
            k_mean_new = np.mean(result.keypoints.xy.cpu().numpy(), axis=1)
            k_mean_old = np.mean(keypoint_list[-1], axis=0)
            distances = np.linalg.norm(k_mean_new - k_mean_old, axis=1)
            player_index = np.argmin(distances)
            keypoint_list.append(result.keypoints.xy[player_index].cpu().numpy())
        else:
    ...
    save_name = video_path.parent / Path(video_path.stem + '_with_keypoint.mp4')
    video_writer = cv2.VideoWriter(save_name, fourcc, fps, (width, height))
    for kp, img in zip(keypoint_list, frame_list):
        img = show_yolo_pose(kp, img)
        video_writer.write(img)
    video_writer.release()
    cap.release()
    return keypoint_list, frame_list
...
if __name__ == '__main__':
    ...
    for video in video_list:
        keypoint_list, frame_list = get_openpose_keypoint(video, ball_data, model_list)
